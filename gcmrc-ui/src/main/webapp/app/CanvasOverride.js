...
OpenLayers.FeaturesIndexer = OpenLayers.Class({
    ...
    initialize: function(yOrdering) {
		this.compare = yOrdering ? 
			OpenLayers.FeaturesIndexer.IndexingMethods.Z_ORDER_Y_ORDER :
			OpenLayers.FeaturesIndexer.IndexingMethods.Z_ORDER_DRAWING_ORDER;
    ...
    insert: function(newFeature, data) {
        ...
        while (rightIndex - leftIndex > 1) {
            ...
            var placement = this.compare(this, newFeature, this.features[this.order[middle]][0]);
...
//    },
    ...
    getZIndex: function(feature) {
		...
		if (zIndex == null) {
			var featureTup = this.features[feature.id];
			zIndex = featureTup[1] && featureTup[1].graphicZIndex;
		}
...
/**
 * Namespace: OpenLayers.FeaturesIndexer.IndexingMethods
 * These are the compare methods for figuring out where a new node should be 
 *     placed within the indexer. These methods are very similar to general 
 *     sorting methods in that they return -1, 0, and 1 to specify the 
 *     direction in which new nodes fall in the ordering.
 */
OpenLayers.FeaturesIndexer.IndexingMethods = {
     ...
     * 
     * Parameters:
     * indexer - {<OpenLayers.FeaturesIndexer>}
     * newNode - {DOMElement}
     * nextNode - {DOMElement}
     * 
     * Returns:
    ...
    Z_ORDER_DRAWING_ORDER: function(indexer, newNode, nextNode) {
        var returnVal = OpenLayers.FeaturesIndexer.IndexingMethods.Z_ORDER(
            indexer, 
            newNode, 
            nextNode
        );
...
OpenLayers.Renderer.Canvas = OpenLayers.Class(OpenLayers.Renderer, {
    ...
    initialize: function(containerID, options) {
		...
		if(options && (options.zIndexing || options.yOrdering)) {
            this.indexer = new OpenLayers.FeaturesIndexer(options.yOrdering);
        }
    },
