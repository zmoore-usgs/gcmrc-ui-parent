...
GCMRC.Graphing = function(hoursOffset) {
	...
	var dealWithResponse = function(graphToMake, data, config, buildGraph) {
		...
		conf.data = data.success.data.map(function(el) {
			...
			var dataColumns = graphToMake.columns.map(function(col) {
		...
		if (hasData) {
			...
			[].push.apply(conf.labels, graphToMake.columns.map(function(el) {
			...
			conf["colors"] = [];
			conf["highlightColor"] = {};
			[].push.apply(conf.colors, graphToMake.columns.map(function(el) {
			...
			conf["series"] = {};
			graphToMake.columns.map(function(el) {
				var param = GCMRC.Page.params[graphToMake.pCode][el.split("!")[0]];
